#!/bin/bash
#SBATCH --job-name=dynamic_ensemble_lgbm_test            # name shown in squeue
#SBATCH --output=logs/%j_dynamic_ensemble_lgbm_test.out  # stdout log (%j = job id)
#SBATCH --error=logs/%j_dynamic_ensemble_lgbm_test.err   # stderr log
...
cmd="python -u -m train_models.dynamic_ensemble --mode prelim --job-id $job_id $resume_flag --n-jobs 2 --checkpoint-every-iters 250 --early-stopping-holdout-frac 0.03"
